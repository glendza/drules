...
def get_rules_and_actions() -> tuple[dict, dict]:
    with open("rulesetJSONState.json") as f:
...
def parse_action(action: dict) -> typing.Callable:
    ...
    match action.get("action"):
        ...
        case "change_state":
            ...
            def fn(c):
                print(c.s[action.get("payload")[0]["state_name"]])
                c.s[action.get("payload")[0]["state_name"]] = action.get("payload")[0][
                    "state_value"
                ]
                print(c.s[action.get("payload")[0]["state_name"]])
                print(lang.get_host().get_state(RULESET_NAME))
...
def run_rules():
    ...
    # )
    lang.update_state("flow", {"status": "start"})
